...
```
deepresearch_agent/integrations/build/
...
`deepresearch_agent/integrations/build` 模块是知识图谱系统的高层构建工具，封装了 `deepresearch_agent/graph` 和 `deepresearch_agent/community` 等底层模块，提供完整的图谱构建、索引创建和增量更新流程。本模块采用了模块化设计，将图谱构建过程分解为多个独立且可组合的步骤，既支持完整的一站式构建，也支持单独执行特定步骤。
...
```python
from deepresearch_agent.integrations.build.incremental_update import IncrementalUpdateManager
...
```bash
# 执行完整构建
python deepresearch_agent/integrations/build/main.py

# 运行增量更新（单次）
python deepresearch_agent/integrations/build/incremental_update.py --once

# 运行增量更新（守护进程模式）
python deepresearch_agent/integrations/build/incremental_update.py --daemon --interval 300
```
