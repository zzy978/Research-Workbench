...
或在 `deepresearch_agent/config/settings.py` 中设置：
```python
...
from deepresearch_agent.community import CommunityDetectorFactory
...
```python
from deepresearch_agent.community import CommunitySummarizerFactory
...
在 `deepresearch_agent/integrations/build/main.py` 中，社区检测和摘要是知识图谱构建的一部分：

```python
from deepresearch_agent.integrations.build.builders import IndexCommunityBuilder
...
```python
from deepresearch_agent.community.detector.base import BaseCommunityDetector
...
```python
from deepresearch_agent.community.summary.base import BaseSummarizer
