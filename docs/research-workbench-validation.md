...
初次验证日期：2026-09-22。实现分支：`codex/research-workbench`。所有自动化及真实模型测试均使用隔离数据库，未修改原项目的研究历史或未跟踪 Skill。

## 2026-09-23：局部失败隔离与部分交付

本轮确定性回归 **396 passed、3 deselected**，另排除下文列出的 Neo4j 收集依赖文件。排除项与初次验证相同，没有删除或弱化相应测试。前端状态测试 **5/5**，TypeScript 检查和生产构建通过，`git diff --check` 通过。

新增测试覆盖：坏引用在第 2 次局部抽取或末尾补查中恢复；持续坏引用、空抽取、检索失败只留下局部缺口；其他字段和对象继续执行；全部检索失败或所有字段都没有可引用结论时仍交付部分报告；连续暂停、异常中断恢复；末尾补查遵守剩余 Token 预算；认证、取消、预算和保存异常整体停止；旧执行器捕获异常后仍不能绕过全局错误状态。

`tests/e2e/research_partial_browser.mjs` 通过真实 API、SQLite、页面及刷新验证：3 个对象中 2 个完成、1 个待补查；矩阵显示“证据校验失败 · 已尝试 3 次”；报告标为部分报告，定稿按钮禁用。截图保存在 `.local-run/partial-e2e/screenshots/partial-matrix.png` 和 `partial-report.png`。本轮模型和检索均使用确定性响应，没有新增真实 LLM/Tavily 调用验证。

独立审查发现并修复了两处边界：检索台账非空但字段没有有效引用时错误进入反复重规划；暂停或中断消耗尝试计数后字段被静默跳过。相关回归已纳入上述结果。

测试仍报告依赖弃用提示及已有 API 测试清理阶段的 aiosqlite 线程警告；前端构建仍有大于 500 kB 的包体提示。因此本轮不宣称全仓库零警告，也不宣称 Neo4j 真实服务验证通过。
